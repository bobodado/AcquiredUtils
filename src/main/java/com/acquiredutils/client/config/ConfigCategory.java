...
public class ConfigCategory {
    ...
    public ConfigCategory addSetting(Setting<?> setting) {
        elements.add(setting);
        return this;
    }

    public ConfigCategory addSection(Section section) {
    ...
    /** Backward compat - top-level settings only */
    public List<Setting<?>> getSettings() {
        List<Setting<?>> settings = new ArrayList<>();
        for (GuiElement e : elements) {
            if (e instanceof Setting<?>) settings.add((Setting<?>) e);
        }
        return settings;
    }
}
