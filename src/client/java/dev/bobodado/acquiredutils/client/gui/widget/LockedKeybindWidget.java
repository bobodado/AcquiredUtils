...
public class LockedKeybindWidget extends AbstractWidget implements KeyListenerSlot.Listener {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.fill(getX(), cbY, getX() + cbSize, cbY + cbSize, COLOR_BOX_BG);
        graphics.renderOutline(getX(), cbY, cbSize, cbSize, COLOR_BORDER);
        if (enabled) {
            graphics.fill(getX() + 2, cbY + 2, getX() + cbSize - 2, cbY + cbSize - 2, COLOR_CHECK_ON);
        }
        ...
        graphics.drawString(font, getMessage(), 0, 0, theme.text, false);
        ...
        int keyColor = listening ? COLOR_LISTENING : (keyCode < 0 ? COLOR_NONE : theme.text);

        graphics.fill(keyBoxX, keyBoxY, keyBoxX + keyBoxW, keyBoxY + height, COLOR_BOX_BG);
        graphics.renderOutline(keyBoxX, keyBoxY, keyBoxW, height, listening ? COLOR_LISTENING : COLOR_BORDER);
        int tw = font.width(keyText);
        graphics.drawString(font, keyText, keyBoxX + (keyBoxW - tw) / 2, getY() + (height - 8) / 2, keyColor, false);
    }
    ...
    public void onClick(MouseButtonEvent event, boolean doubleClick) {
        ...
        int cbSize = s(CHECKBOX_SIZE);
        int cbY = getY() + (height - cbSize) / 2;
        boolean onCheckbox = mouseX >= getX() && mouseX < getX() + cbSize
        ...
        int keyBoxW = s(KEY_BOX_WIDTH);
        int keyBoxX = getX() + width - keyBoxW;
        boolean onKeyBox = mouseX >= keyBoxX && mouseX < keyBoxX + keyBoxW
